...
if (version) {
  console.log(`Переданная версия: ${version}`);
} else {
...
async function fetchHTML(url) {
  try {
    ...
    console.error(`Ошибка при загрузке HTML по адресу ${url}: ${error.message}`);
    throw error;
  }
}
...
async function getDetails(target, subtarget) {
  ...
  $('a').each((index, element) => {
    ...
    if (name && name.startsWith('kernel_')) {
      const vermagicMatch = name.match(
        /kernel_\d+\.\d+\.\d+(?:-\d+)?[-~]([a-f0-9]+)(?:-r\d+)?_([a-zA-Z0-9_-]+)\.ipk$/
      );
      if (vermagicMatch) {
        vermagic = vermagicMatch[1];
        pkgarch = vermagicMatch[2];
        console.log(
          `Нашёл vermagic: ${vermagic}, pkgarch: ${pkgarch} для target: ${target}, subtarget: ${subtarget}`
        );
      }
    }
  });
